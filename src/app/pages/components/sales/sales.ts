...
export class Sales implements OnInit, AfterViewInit {
  ...
  async importFromExcel(event: any) {
    ...
    try {
      // Obtener todos los saleos existentes para validar duplicados
      const response = await this.saleSvc.getSales('-', 0, 9999, '', '').toPromise();
      const existingSales = response?.content || [];

      // Configurar la importación
      const importConfig = {
        columns: this.excelColumns,
        allowDuplicates: false,
        existingData: existingSales,
        duplicateKey: 'name'
      };
      ...
      const shouldProceed = await this.excelService.showImportSummary(validationResult, 'ventas');

      if (shouldProceed) {
        // Ejecutar la importación
        const importResult = await this.excelService.executeImport(
          validationResult.validData,
          (sale: Sale) => this.saleSvc.createSale(sale).toPromise(),
          'ventas'
        );

        // Mostrar resultado
        this.excelService.showImportResult(importResult, 'ventas');
  ...
  openImportHelpModal() {
    this.dialog.open(ExcelHelpModalComponent, {
      ...
      data: {
        columns: this.excelColumns,
        entityName: 'ventas',
        templateFileName: 'plantilla_importar_ventas',
        exampleData: [
          {
            'Nombre': 'Ventas de Ejemplo',
            'ID': '1001123456',
